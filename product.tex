...
\prod_a \ket{\nu_a} \ket{J, K, m, \pm}
\]
where $\ke{J, K, m, \pm}$ is the rotational basis function, defined in X, and $\ket{\nu}_a is the $\nu$th excitation of the $a$ coordinate, obtained by solving the 1D Schrodinger equation
\[
\pi_a G^{\text 1D}_{aa} \pi_a + U^{\text 1D} + V^{\text 1D}
\]
where $V$ is the potential. The 1D versions of the kinetic energy operator components are constructed by constraining the other $3N-7$ coordinates to their equilibrium values. The former method stores certain elemantary integrals, namely \[ \begin{split}
		&\matrixel{\nu_i}{(q_i)^j}{\nu_i'} \\
		&\matrixel{\nu_i}{(q_i)^j\hat{\pi}_i}{\nu_i'} \\
		&\matrixel{\nu_i}{\hat{\pi}_i (q_i)^j}{\nu_i'} = - \matrixel{\nu_i}{(q_i)^j \hat{\pi}_i}{\nu_i'}\\
